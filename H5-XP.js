...
let httpRequestDependecies = {
    ...
    "timePlayed": [prefs.showLifetimeRate, prefs.showCustomAverageRate, prefs.showPlaytime]
}

if (fm.fileExists(fm.documentsDirectory() + "/H5-XP-Prefs.json")) { log("🟠 There is a \"H5-XP-Prefs.json\" file located in /iCloud/Scriptable/. This file is not needed as of H5-XP version 1.5 and can be removed.") }
if (fm.bookmarkExists("H5-XP-StoredXP.json")) { log("🟠 There is a Scriptable bookmark named \"H5-XP-StoredXP.json\". This is not needed as of H5-XP version 1.5 and can be removed. There may also be a \"H5-XP-StoredXP.json\" file in /iCloud/Shortcuts/. This is no longer needed either and can be removed.") }
...
if (prefs.showPlaytime) { addTextElement("PLAYTIME", hoursToHHMM(durationToHours(WZDuration) + durationToHours(ArenaDuration) - hoursAtDayStart)) }
...
if (!fm.isFileDownloaded(fm.documentsDirectory() + "/H5-XP-bgImage.png")) {
    if (prefs.useLog) { "🟡 The background image is saved to iCloud, but needs to be downloaded. Doing that now..." }
    fm.downloadFileFromiCloud(fm.documentsDirectory() + "/H5-XP-bgImage.png")
}
